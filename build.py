...
def main():
    #1. grab a list of all content html files
    all_content_files = glob.glob("content/*.html")
    #2. use a for loop to iterate through each
    # have a separate os function here?
    for page in all_content_files:
        file_path = page["name of page I'm still unsure of"]
        file_name = os.path.basename(file_path)
        name_only, extension = os.path.splitext(file_name)
    for page in all_content_files:
        #3. read the base template file
        template_html = open("templates/base.html").read()
        #4. store each content file as a variable
        content_html = open(page['what should this refer to?']).read()
        #5. reassign to a variable so the two pieces
        # of content can be put together with jinja templating
        template = Template(template_html)
        #6. use render method to template in info
        template.render(
            title="name_only", #use os here or in a separate function?
            content=page['how to refer to this?'],
        )
        #7. save into docs folder... how?
...
#____________________________________________________
    ...
    #         open('docs/blog.html', 'w+').write(finished_blog_page) #appending
    # blog_main()
